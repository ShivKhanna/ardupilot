...
const AP_Param::GroupInfo AP_BattMonitor::var_info[] = {
    ...
    // @Group: _
    // @Path: AP_BattMonitor_MAVLink.cpp
...
AP_BattMonitor::init()
{
    ...
    for (uint8_t instance=0; instance<AP_BATT_MONITOR_MAX_INSTANCES; instance++) {
        ...
        switch (allocation_type) {
            ...
            case Type::MAVLink:
                drivers[instance] = NEW_NOTHROW AP_BattMonitor_MAVLink(*this, state[instance], _params[instance]);
                break;
...
bool AP_BattMonitor::handle_mavlink_battery_status(const mavlink_message_t &msg)
{
    // Ignore our own BATTERY_STATUS if it is looped back through a MAVLink router.
    ...
    for (uint8_t i = 0; i < _num_instances; i++) {
        if (drivers[i] == nullptr ||
            configured_type(i) != Type::MAVLink ||
            allocated_type(i) != Type::MAVLink) {
            continue;
        }
        handled |= static_cast<AP_BattMonitor_MAVLink *>(drivers[i])->handle_battery_status(msg);
    }
